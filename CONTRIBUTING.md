...
3) Appliquer les scripts SQL Supabase nécessaires (`supabase_setup.sql`, migrations clients/finance/goals) si vous utilisez une base vierge.

### Branches & commits
- Branche de feature : `feature/<sujet>` ; correctif : `fix/<sujet>`.
- Convention de commit : `type(scope): message` (ex. `feat(finance): add payments tab`).
...
- Conversions camelCase ↔ snake_case centralisées dans `supabaseService.ts` / `financialService.ts` / `goalsService.ts`.
...
- Parcours manuel minimal avant PR :  
  ...
  - Créer un devis, générer PDF; créer une facture + paiement; consulter les rapports.  
...
- Observabilité (Sentry/LogRocket) et validation métier (paiements vs montants facturés).
