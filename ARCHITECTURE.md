...
- **Frontend** : React 19 + TypeScript, Vite, Tailwind 4, composants métier (Dashboard, Missions, Agenda, Finance, Kanban, PWA prompt).
...
- **Shell & navigation** : `App.tsx` gère les vues (`dashboard`, `missions`, `calendar`, `finance`, `kanban`), l’auth Supabase, le lazy loading et le modal d’édition de mission.
- **Services** :
  ...
  - `financialService.ts` : CRUD invoices, quotes, payments, rapports financiers.
  ...
  - `pdfService.ts` : génération PDF (factures/devis) via jspdf.
- **UI clés** :
  - Missions : `MissionForm`, `MissionsList`, `KanbanView`, `CalendarView` (filtre, heatmap CA).
  - Finance : `FinanceView`, `InvoiceForm`, `QuoteForm`, `PaymentForm`, `FinancialCharts`.
...
4. Dashboard/Calendar/Kanban lisent le state en mémoire ; export CSV/JSON exploite le même state.

### Sécurité
- RLS activé sur toutes les tables (missions, clients, invoices, quotes, payments, goals). Filtre par `auth.uid() = user_id` sur SELECT/INSERT/UPDATE/DELETE.
...
- Lazy loading des vues lourdes (Dashboard, Calendar, Finance, Kanban).
