...
import { Quote, QuoteItem, Mission } from '../types';
...
const QuoteForm: React.FC<QuoteFormProps> = ({ isOpen, onClose, onSave, initialData, missions }) => {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setQuoteNumber(generateDocumentNumber('DEV', year, sequence));
      setMissionId('');
      setClient('');
      setClientAddress('');
      setClientEmail('');
  ...
  const validate = () => {
    ...
    if (!quoteNumber.trim()) newErrors.quoteNumber = 'Le numéro de devis est requis';
    if (!client.trim()) newErrors.client = 'Le client est requis';
    if (!issueDate) newErrors.issueDate = 'La date d\'émission est requise';
    if (!validUntil) newErrors.validUntil = 'La date de validité est requise';
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className={`glass-button w-full px-4 py-2 rounded-lg ${errors.quoteNumber ? 'border-red-500' : ''}`}
                required
              />
            </div>
                  ...
                  <div className="grid grid-cols-12 gap-3 items-end">
                    <div className="col-span-12 md:col-span-5">
                      ...
                      <input
                        ...
                        className="glass-button w-full px-3 py-2 rounded-lg text-sm"
