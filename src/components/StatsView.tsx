...
const CHART_COLORS = [
  'var(--primary)', // indigo-400   — primary brand
  'var(--success)', // emerald-400  — success/income
  'var(--warning)', // amber-400    — gold/value
  'var(--primary)', // blue-400
  '#a78bfa', // pink-400
  'var(--success)', // cyan-400
  '#a78bfa', // violet-400
  'var(--success)', // green-400
];

const AXIS_STYLE = { fill: 'var(--text-tertiary)', fontSize: 11, fontFamily: 'Inter, system-ui' };
...
const GlassRevenueTooltip: React.FC<BarTooltipProps> = ({
  ...
  return (
    <div className="glass-elevated rounded-[var(--radius-md)] border border-[var(--border-strong)] px-3.5 py-3 shadow-xl min-w-[150px]">
      <p className="text-[9px] font-bold text-[var(--text-tertiary)] uppercase tracking-widest mb-2.5">
        {label}
      </p>
      <p className="num-financial text-base font-bold text-[var(--warning)] leading-none">
        {hidePrices
          ? '*** €'
          : `${payload[0].value.toLocaleString('fr-FR', { maximumFractionDigits: 0 })} €`
        }
      </p>
      <p className="text-[10px] text-[var(--text-tertiary)] mt-1">Chiffre d'affaires</p>
    </div>
  );
};
...
const GlassCountTooltip: React.FC<CountTooltipProps> = ({ active, payload, label }) => {
  ...
  return (
    <div className="glass-elevated rounded-[var(--radius-md)] border border-[var(--border-strong)] px-3.5 py-3 shadow-xl">
      <p className="text-[9px] font-bold text-[var(--text-tertiary)] uppercase tracking-widest mb-2.5">
        {label}
      </p>
      <p className="num-financial text-base font-bold text-[var(--success)] leading-none">
        {n} <span className="text-[11px] font-normal text-[var(--text-tertiary)]">mission{n > 1 ? 's' : ''}</span>
      </p>
    </div>
  );
};
...
const GlassPieTooltip: React.FC<PieTooltipProps> = ({ active, payload, hidePrices }) => {
  ...
  return (
    <div className="glass-elevated rounded-[var(--radius-md)] border border-[var(--border-strong)] px-3.5 py-3 shadow-xl">
      <p className="text-[11px] font-bold text-[var(--text-secondary)] mb-1.5 leading-snug">{entry.name}</p>
      <p className="num-financial text-sm font-bold text-[var(--warning)] leading-none">
        {hidePrices
          ? '*** €'
    ...
    <div className="space-y-6 pb-24 md:pb-8 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
        <div>
          <div className="flex items-center gap-3 mb-1.5">
            <div className="p-2.5 rounded-[var(--radius-md)] bg-[var(--primary-light)] border border-[var(--primary)] shadow-lg shadow-[var(--primary-glow)]">
          ...
          <p className="text-xs text-[var(--text-tertiary)] ml-[52px]">
            Période :{' '}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        <KpiCard
          ...
          iconBg="bg-[var(--primary-light)] border-[var(--primary)]"
          gradient="from-[var(--primary-light)] via-[var(--primary-light)] to-transparent"
        ...
        />
        <KpiCard
          icon={<Clock className="w-4 h-4 text-[var(--success)]" />}
          iconBg="bg-[var(--success-light)] border-[var(--success)]"
          gradient="from-[var(--success-light)] via-[var(--success-light)] to-transparent"
        ...
        />
        <KpiCard
          icon={<CalendarRange className="w-4 h-4 text-[#a78bfa]" />}
          iconBg="bg-[var(--primary-light)] border-[var(--primary)]"
          gradient="from-[var(--primary-light)] via-[var(--primary-light)] to-transparent"
          border="border-[var(--primary)]"
          label="vs mois préc."
          ...
          valueClass={monthlyComparison.isPositive ? 'text-[var(--success)]' : 'text-[var(--danger)]'}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="glass rounded-[var(--radius-lg)] p-4 lg:col-span-2">
          <ChartHeader
            ...
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <RechartsTooltip
                  ...
                  cursor={{ fill: 'var(--primary-light)', radius: [4, 4, 0, 0] }}
                />
                <Bar
                  dataKey="revenue"
                  name="CA"
                  ...
                  activeBar={{ fill: 'url(#revenueGradientActive)', stroke: 'var(--primary-glow)', strokeWidth: 1 }}
                  isAnimationActive
        ...
        <div className="glass rounded-[var(--radius-lg)] p-4">
          <ChartHeader
            ...
            iconBg="bg-[var(--warning-light)] border-[var(--warning)]"
            title="Top clients"
            sub="Répartition CA par client"
          />

          {clientRevenueData.length === 0 ? (
            <div className="flex items-center justify-center h-56 text-xs text-[var(--text-tertiary)] italic">
                ...
                {clientRevenueData.map((client, index) => (
                  <div
                    ...
                    className="flex items-center justify-between gap-2 px-2.5 py-1.5
                               rounded-[var(--radius-md)] bg-[var(--bg-tertiary)] border border-[var(--border-default)]
                  ...
                  >
                    ...
                    <span className="num-financial text-[11px] font-bold text-[var(--warning)] shrink-0">
                      {hidePrices
                        ? '***'
      ...
      <div className="glass rounded-[var(--radius-lg)] p-4">
        <ChartHeader
          icon={<BarChart3 className="w-4 h-4 text-[var(--success)]" />}
          iconBg="bg-[var(--success-light)] border-[var(--success)]"
            ...
            >
              <defs>
                <linearGradient id="weekdayGradient" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%"   stopColor="var(--success)" stopOpacity={0.90} />
                  <stop offset="100%" stopColor="var(--primary)" stopOpacity={0.30} />
                </linearGradient>
              ...
              <RechartsTooltip
                ...
                cursor={{ fill: 'var(--success-light)', radius: [4, 4, 0, 0] }}
              />
              <Bar
                dataKey="count"
                name="Missions"
                ...
                activeBar={{ fill: 'url(#weekdayGradientActive)', stroke: 'var(--success-light)', strokeWidth: 1 }}
                isAnimationActive
                animationDuration={700}
                animationEasing="ease-out"
              />
...
const KpiCard: React.FC<KpiCardProps> = ({
  icon, iconBg, gradient, border, label, value, sub, valueClass = 'text-[var(--text-primary)]',
}) => (
  <div className={`glass rounded-[var(--radius-lg)] p-4 border-t-2 bg-gradient-to-br to-transparent ${gradient} ${border}`}>
    <div className={`inline-flex p-2 rounded-[var(--radius-md)] border mb-3 ${iconBg}`}>
      {icon}
    </div>
    <p className={`gradient-text num-financial text-2xl md:text-3xl font-bold tracking-tight leading-none mb-1 ${valueClass}`}>
      {value}
    </p>
    <p className="text-[10px] font-semibold uppercase tracking-wider text-[var(--text-tertiary)] leading-none mb-2">
      {label}
    </p>
    {sub && (
      <p className="text-[10px] text-[var(--text-tertiary)] leading-snug">{sub}</p>
    )}
  </div>
);
...
const ChartHeader: React.FC<ChartHeaderProps> = ({ icon, iconBg, title, sub }) => (
  <div className="flex items-center gap-2.5 mb-5">
    <div className={`p-2 rounded-[var(--radius-md)] border ${iconBg}`}>
      {icon}
    </div>
    <div>
      <h2 className="font-display text-sm font-bold text-[var(--text-primary)] leading-tight">{title}</h2>
