        ...
        <div className="flex flex-col md:flex-row gap-2.5 items-center justify-between">
          <div className="relative w-full md:max-w-xs">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-2 bg-[var(--bg-tertiary)] border border-[var(--border-default)] rounded-[var(--radius-md)] focus:ring-2 focus:ring-[var(--primary)] focus:border-[var(--primary)] outline-none transition-all text-[11px] text-[var(--text-primary)] placeholder-[var(--text-tertiary)] font-medium"
            />
          </div>
          ...
          <div className="flex gap-2 w-full md:w-auto">
            {uniqueMonths.length > 0 && (
              <div className="relative w-full md:w-auto">
                ...
                >
                  <option value="all" className="bg-[var(--bg-elevated)] text-[var(--text-secondary)]">Tous les mois</option>
                  {uniqueMonths.map(monthKey => {
                    const [year, month] = monthKey.split('-');
                    const date = new Date(parseInt(year), parseInt(month) - 1);
                    return (
                      <option key={monthKey} value={monthKey} className="bg-[var(--bg-elevated)] text-[var(--text-secondary)] capitalize">
            ...
            {uniqueClients.length > 0 && (
              <div className="relative w-full md:w-auto">
                <select
                  ...
                  className="w-full md:w-auto appearance-none pl-3 pr-8 py-1.5 bg-[var(--bg-tertiary)] border border-[var(--border-default)] rounded-[var(--radius-md)] text-[10px] font-medium text-[var(--text-secondary)] focus:ring-2 focus:ring-[var(--primary)] focus:outline-none cursor-pointer"
                >
                  <option value="all" className="bg-[var(--bg-elevated)] text-[var(--text-secondary)]">Tous clients</option>
                  {uniqueClients.map(client => (
                    <option key={client} value={client} className="bg-[var(--bg-elevated)] text-[var(--text-secondary)]">{client}</option>
                  ))}
                </select>
                <div className="absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none text-[var(--text-tertiary)]">
                  <Briefcase size={10} />
                ...
                {sortedGroupKeys.map(monthKey => {
                  ...
                  return (
                    <React.Fragment key={monthKey}>
                      ...
                      {missionsInMonth.map((mission) => (
                        <tr key={mission.id} className="group hover:bg-white/[0.04] transition-all duration-300">
          ...
          <div className="glass w-full max-w-md rounded-[var(--radius-lg)] overflow-hidden shadow-2xl animate-scale-in border border-[var(--primary)]">
            <div className="flex items-center justify-between p-5 border-b border-[var(--border-default)]">
              ...
              <button
                ...
                className="p-2 hover:bg-[var(--bg-elevated)] rounded-[var(--radius-md)] transition-colors text-[var(--text-tertiary)] hover:text-[var(--text-secondary)]"
            ...
            <div className="flex justify-end gap-3 p-5 border-t border-[var(--border-default)] bg-[var(--bg-secondary)]">
              <button
                onClick={() => setShowPdfModal(false)}
